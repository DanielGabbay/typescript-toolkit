...
describe('performance utilities', () => {
  ...
  describe('benchmarkCompare', () => {
    ...
    it('should handle async functions in comparison', async () => {
      ...
      expect(results[0].opsPerSecond).toBeGreaterThanOrEqual(results[1].opsPerSecond);
      // Just verify both have reasonable performance metrics (timing can be inconsistent)
      expect(results.find(r => r.name === 'async-1')?.opsPerSecond).toBeGreaterThan(0);
      expect(results.find(r => r.name === 'async-2')?.opsPerSecond).toBeGreaterThan(0);
    });
